...
class Item : virtual public Thing
{
	public:
		...
		const uint16_t getPokemonHeldX() {
			return getAttributes()->getIntAttr(ITEM_ATTRIBUTE_HELDX);
		}

		const uint16_t getPokemonHeldY() {
			return getAttributes()->getIntAttr(ITEM_ATTRIBUTE_HELDY);
		}

		const uint16_t getPokemonHeldU() {
			return getAttributes()->getIntAttr(ITEM_ATTRIBUTE_HELDU);
		}
