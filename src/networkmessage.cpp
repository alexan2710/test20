...
void NetworkMessage::addItem(const Item* item)
{
	...
	if (!value) {
		...
		uint8_t isPokeball = item->isPokeballItem() ? 0x1 : 0x0;
		addByte(isPokeball);
		if (isPokeball) {
			addString(const_cast<Item*>(item)->getPokemonName());
			addByte(const_cast<Item*>(item)->getPokemonBoost());
			add<uint16_t>(const_cast<Item*>(item)->getPokemonHeldX());
			add<uint16_t>(const_cast<Item*>(item)->getPokemonHeldY());
			add<uint16_t>(const_cast<Item*>(item)->getPokemonHeldU());
		}
