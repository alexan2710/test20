		...
		if tile2:getHouse() then
			if tile then
				if not tile:getHouse() then
					...
					if itemType and itemType.isHolder and itemType:isHolder() then
						self:sendTextMessage(MESSAGE_EVENT_DEFAULT, 'Desculpe, mova o holder para sua backpack.')
						return false
					end
				end
			end
			...
			if holderType and holderType:isHolder() then
				...
				if not creatureId or creatureId == 0 then
					...
					if checkTile then
						...
						local creatureName = string.format("%s", string.upper(string.sub(spawnName, 1, 1)) .. string.sub(spawnName, 2))
