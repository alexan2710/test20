...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if target:isItem() then
        if target:isItem() then
            ...
            --@ rock smash
            if item and table.contains(rocks, item:getId()) then
                local canSmash = monsterType:canSmash()
                if canSmash then
                    if not table.contains(summon:getEvents(CREATURE_EVENT_THINK), "thinkBreakOrderSmash") then
            ...
            --@ cut
            if item and table.contains(bushes, item:getId()) then
                local canCut = monsterType:canCut()
                if canCut then
                    if not table.contains(summon:getEvents(CREATURE_EVENT_THINK), "thinkBreakOrderCut") then
                        summon:setOrderPosition(toPosition)
            ...
            if table.contains(holes, groundId) then
                ...
                if canDig then
                    if not table.contains(summon:getEvents(CREATURE_EVENT_THINK), "thinkBreakOrderDig") then
                        summon:setOrderPosition(toPosition)
    ...
    if target:isCreature() then
		if target == player then
            ...
            if (canFly >= 1) then
                player:sendExtendedOpcode(ExtendedOPCodes.CODE_FLY, "open")
            end
		end
	end
	return true
...
function thinkBreakOrderDig.onThink(pokemon, interval)
    ...
    if toPosition then 
        ...
        if pokemonPosition:getDistance(toPosition) < 2 then
            ...
            local groundId = ground:getId()
            if not table.contains(holes, groundId) then
                pokemon:unregisterEvent("thinkBreakOrderDig")
                return true
            end
