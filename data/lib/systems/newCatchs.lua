...
function Player:sendBrokesToPlayer()
    ...
    for pokeName, info in pairs(INFOS_CATCH) do
        ...
        if monsterType then
            ...
            if table.contains(LOWER_PALWORLD_MONSTERS, string.lower(pokeName)) then
                type = "palworld"
                type2 = "palworld"
            else
...
function Player:resetCatchTry(pokeName)
 ...
 local function sendOpcodeJson(player, action, data)
 	...
 	local buffer = json.encode({ type = action, data = data })
 	local s = {}
