...
balls = {
	pokeball = {emptyId = 26662, usedOn = 26677, usedOff = 26672, effectFail = 1521, effectSucceed = 1522, missile = 58, effectRelease = 1520, chanceMultiplier = 1000.0}, --  0.25
	...
	super = {emptyId = 26659, usedOn = 26669, usedOff = 26674, effectFail = 1527, effectSucceed = 1528, missile = 60, effectRelease = 1526, chanceMultiplier = 2.0},
...
function doReleaseSummon(cid, pos, effect, message, missile)
	...
	if monster ~= nil then
		...
		if message then
			...
			player:sendExtendedOpcode(ExtendedOPCodes.CODE_FLY, "close")
		end		
...
function Position:sendAnimatedNumber(message_type, text, value, color)
	...
	assert(table.contains(valid_message_types, message_type), "invalid 'message_type', must output to server log")
...
function Player:canFlyUp()
	...
	if tmp and tmp:getGround() and table.contains(waterIds, tmp:getGround():getId()) then
		return false
	end
	return true
end
...
function Player:sendSoundAround(data)
	local data = data or ""
	local position = self:getPosition() -- posição do player alvo.
	local minRangeX = 0
	local maxRangeX = 17
	local minRangeY = 0
	local maxRangeY = 13
...
function addOnlineBonus(playerId)
	...
	if player then
		...
		if player:getStorageValue(72703) >= 60 then
			player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE,"Por completar uma hora online sem deslogar, você ganhou 1 Online Point. Total: " ..player:getStorageValue(71344) .. ".")
