...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local pontos = 0
	if table.contains(LOWER_PALWORLD_MONSTERS, string.lower(name)) then
		type = "palworld"
		type2 = "palworld"
	end
	...
	if table.contains(LOWER_PALWORLD_MONSTERS, string.lower(name)) then
		if not table.contains(ALLOWED_POKEBALLS_PALWORLD, item.itemid) then
			player:sendCancelMessage("Você só pode capturar pals com esferas.")
			return true
		end
	end

	if table.contains(LOWER_MASTERBALL_BLOCKED, string.lower(name)) then
	...
	if math.random(0, 10000) <= chance or INFOS_CATCH[string.lower(name)] and (pontos+player:getCatchPoints(name)) >= INFOS_CATCH[string.lower(name)].pontos then
		if player:getSlotItem(CONST_SLOT_BACKPACK) and player:getSlotItem(CONST_SLOT_BACKPACK):getEmptySlots() >= 1 and player:getFreeCapacity() >= 1 then
			-- Adicionar o Pokémon à mochila com o atraso
			addEvent(function()
				player:addPokemon(name, item.itemid)  -- Adiciona o Pokémon à mochila
			end, delayMessage)
		else
			-- Adicionar o Pokémon ao Depot com o atraso
			addEvent(function()
				player:addPokemon(name, item.itemid)  -- Adiciona o Pokémon ao Depot
			end, delayMessage)
		end
